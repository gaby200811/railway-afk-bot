...
class BotSession {
  ...
  stopMovement() {
    ...
    this.movementTimers = [];
    if (this.bot && this.activeDirection) this.bot.setControlState(this.activeDirection, false);
  ...
  startViewer() {
    ...
    try {
      ...
      mineflayerViewer(this.bot, { port: this.viewerPort, firstPerson: false, viewDistance: 4 });
  ...
  createBot() {
    if (this.bot || !this.shouldRun) return;
    ...
    bot.once('spawn', () => {
      ...
      this.startViewer();
      this.startWatchdog();
      this.movementTimers.push(setTimeout(() => this.randomMovement(), 1000));
    });
    ...
    bot.on('goal_reached', goal => {
      ...
      this.navigationMode = 'idle';
      if (this.shouldRun) {
        this.movementTimers.push(setTimeout(() => this.randomMovement(), wasRoute ? 1000 : 2000));
      }
    });
    ...
    bot.on('path_reset', reason => {
      ...
      // freeze AFK movement with no retry. Now we retry on any reason.
      if (this.shouldRun) {
        this.movementTimers.push(setTimeout(() => this.randomMovement(), MOVEMENT_STUCK_RETRY_MS));
      }
    });
    ...
    bot.on('end', () => {
      ...
      if (this.shouldRun) {
        this.addLog('Reconnecting in 5 seconds...', 'system');
        this.reconnectTimer = setTimeout(() => this.createBot(), 5000);
      }
    ...
    bot.on('kicked', reason => this.addLog(`Bot was kicked: ${JSON.stringify(reason)}`, 'error'));
  }
