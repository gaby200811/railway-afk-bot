...
class BotSession {
  constructor(username, io) {
    ...
    this.state = {
      ...
      useProxy: Boolean(saved.useProxy),
  ...
  startViewer() {
    if (this.viewerPort) return { ok: true };
    if (!this.bot?.entity) return { ok: false, error: 'Bot is not connected' };
    try {
      ...
      this.addLog('Live 3D view started', 'success');
      return { ok: true };
    } catch (err) {
      this.viewerPort = null;
  ...
  stopViewer() {
    if (this.viewerPort) {
      ...
      this.viewerPort = null;
      this.addLog('Live 3D view stopped', 'system');
    }
  }
  ...
  async createBot() {
    ...
    // Proxy is used when either the user has manually enabled it in Server
    // settings, or the automatic IP-limit fallback kicked in for this
    // session. Manual enable takes priority in the log message since it was
    // a deliberate choice, not a reaction to a kick.
    if (this.state.useProxy && !process.env.MC_PROXY) {
      this._connecting = false;
      this.state.status = 'offline';
      this.addLog('Proxy is enabled in Server settings, but no MC_PROXY is configured on the host. Set MC_PROXY or turn the proxy toggle off.', 'error');
      return;
    }

    let proxyConnector;
    try {
      const useProxy = this.state.useProxy || this.proxyFallbackActive;
      const proxyAddress = useProxy ? process.env.MC_PROXY : undefined;
      proxyConnector = createProxyConnector(proxyAddress, this.state.host, this.state.port);
      if (proxyConnector) this.addLog(this.state.useProxy ? 'Connecting through the configured SOCKS5 proxy' : 'Using SOCKS5 fallback proxy', 'system');
    } catch (err) {
      this._connecting = false;
      this.state.status = 'offline';
    ...
    bot.once('spawn', () => {
      ...
      // PERF: the live viewer used to auto-start here for every bot the
      // moment it connected. prismarine-viewer runs a full chunk-meshing
      // renderer + its own HTTP/WS server per instance, so with several
      // bots online that's several of those running in the background even
      // when nobody has the 3D view open. It now only starts on request,
      // from the /api/viewer/start route below, and stops when the user
      // closes it or the bot disconnects. viewerAvailable here just means
      // "connected enough to view", not "viewer running".
  ...
  updateConfig({ host, port, username, version, password, useProxy }) {
    ...
    this.state.useProxy = Boolean(useProxy);
